...
def mean_squared_error(y1, y2):
    ...
    return msq  # replace this with your answer.
...
def predict(x, w):
    poly_order = w.size-1;
    X = polynomial_design_matrix(x, order=poly_order);
    y = np.matmul(X,w);
    return y;

def poly_mse_train_vs_test(training_x, training_y, test_x, test_y, order):
    p = LS_poly(training_x, training_y, order, 0)
    d = predict(test_x, p)
    return mean_squared_error(test_y, d)
